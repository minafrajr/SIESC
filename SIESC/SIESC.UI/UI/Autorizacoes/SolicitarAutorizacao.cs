	...
	public partial class SolicitarAutorizacao : SIESC.UI.base_UI
	{
		...
		private void btn_autorizar_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					if (Mensageiro.MensagemPergunta($"Já existe a autorização no sistema para este funcionário!{Environment.NewLine}Deseja Atualizar?{Environment.NewLine}SIM - Atualiza {Environment.NewLine}NÃO - Grava uma nova autorização") == DialogResult.Yes)
					{
		...
		{
			if (Mensageiro.MensagemPergunta("Deseja imprimir a Carteirinha?").Equals(DialogResult.Yes))
			{
		...
		private void btn_excluir_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (Mensageiro.MensagemExclusao(null) == DialogResult.Yes)
				{
			...
			{
				Mensageiro.MensagemAviso(exception.Message);
			}
		}

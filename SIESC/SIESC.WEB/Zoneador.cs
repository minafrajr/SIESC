        ...
        public static string[] Georreferenciar(string cep, string numLogradouro)
        {
            ...
            try
            {
                using (WebClient wc = new WebClient())
                {
                    ...
                    HttpRequestHeader header = HttpRequestHeader.UserAgent;
                    wc.Headers.Add(header, "SIESC/" + versao);

                    string json = wc.DownloadString("https://sistemas.betim.mg.gov.br/api/geocode/geocode?epsg=4326&key=" + Settings.Default.georrefKey + "&cep=@cep&number=@numlograd".Replace("@cep", cep).Replace("@numlograd", numLogradouro));

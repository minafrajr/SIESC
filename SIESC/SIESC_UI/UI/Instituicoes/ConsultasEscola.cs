...
{
	public partial class ConsultasEscola : SIESC_UI.base_UI
	{
		/// <summary>
		...
		public ConsultasEscola(Principal_UI form_principal)
		{
			...
			this.frm_principal = form_principal;
		...
		private void btn_editarescola_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (id > 0 || id != null)
				{
					foreach (Form mdiChild in frm_principal.MdiChildren)
					{
						if (mdiChild.GetType() == typeof(CadastrarEscola))
							mdiChild.Close();
					}
					CadastrarEscola frm_cadastraescola = new CadastrarEscola(id.ToString(), frm_principal);
					frm_cadastraescola.MdiParent = frm_principal;
